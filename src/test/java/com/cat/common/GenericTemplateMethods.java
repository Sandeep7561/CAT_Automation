...
import com.cat.utility.WebAppCommon;
...
public class GenericTemplateMethods extends WebAppCommon {
	...
	public static void runCertRadioButtonORCheckBox() throws Exception {
		try {
			...
			for (int l=1; l>=1; l++)
			{
				if (verifyElementPresent("runSaqORGraphicalSaqOption" + l + "") || verifyElementPresent("RadioORCheckBox" + l + ""))
				{
					if (verifyElementPresent("runSaqORGraphicalSaqOption" + l + ""))
					{
						clickElementText("runSaqORGraphicalSaqOption" + l + "");
					} else if (verifyElementPresent("RadioORCheckBox" + l + ""))
					{
						clickElementText("RadioORCheckBox" + l + "");
					}
				...
				{
					if (verifyElementPresent("runSaqORGraphicalSaqOption" + (l=1) + ""))
					{
						clickElementText("runSaqORGraphicalSaqOption" + l + "");
					} else if (verifyElementPresent("RadioORCheckBox" + l + ""))
					{
	...
	public static void runCertDropDown() throws Exception {
		try {
			...
			{
				try {
					if (verifyElementPresent("DropDownBox"))
					{
						new Select (driver.findElement(objmap.getLocator("DropDownBox"))).selectByIndex(l);
					} else if (verifyElementPresent("DropDownMenu"))
					{
						clickElement("DropDownMenu");
						Thread.sleep(2000);
						new Select (driver.findElement(objmap.getLocator("DropDownList"))).selectByIndex(l);
					}
	...
	public static void checkViewCertOnResultsPage() throws Exception {
		try {
			String CurrentPageURL = driver.getCurrentUrl();
			if (!CurrentPageURL.contains("FluidX2.0"))
			{
				logINFO("Print Certificate button available on page");
			} else {
				TakeScreenshot("Pass");
				if (verifyElementPresent("ConnectionLostMessageBox"))
				{
	...
	public static void clickTakeSurveyButton() throws Exception {
		try {
			...
			if (!CurrentPageURL.contains("FluidX2.0"))
			{
				logINFO("Take Survey button available on page");
			} else {
				TakeScreenshot("Pass");
				String winHandleBefore = driver.getWindowHandle();
		...
		try {
			if (verifyElementPresent("ConnectionLostMessageBox"))
			{
				...
				do {
					driver.get(LCECURL);
					Thread.sleep(3000);
				} while (!verifyElementPresent("FluidXHTMLPage") || verifyElementPresent("ConnectionLostMessageBox"));
				TakeScreenshot("Pass");
			}
		} catch (Exception e) {
	...
	public static void runHTML5course(String XlsName, String SheetName, int StartRowNumber, int EndRowNumber) throws Exception {
		try {
			...
			for (int j=StartRowNumber-1; j<=EndRowNumber-1; j++)
			{
				...
				// Run Course on ScormCloud LMS
				//runScormCloudCourseLaunch();
				
				// Run Course in CATPreview
				//runCATpreviewCourseLaunch();
				//runFluidXCourse();
				
				// Run one Template Page in CATPreview
				//runCATpreviewCourseLaunch();
				//catPreviewJumptToPage();
	...
	public static void runCATpreviewCourseLaunch() throws Exception {
		try {
			if (enviornment.equals("CATpreview"))
			{
				driver.get(siteName);
				logINFO("" + siteName +"");
			}
			driver.findElement(objmap.getLocator("CATUserName")).clear();
			driver.findElement(objmap.getLocator("CATUserName")).sendKeys(userName);
			driver.findElement(objmap.getLocator("CATPassword")).clear();
			driver.findElement(objmap.getLocator("CATPassword")).sendKeys(password);
			TakeScreenshot("Pass");
			clickElement("CATLogIn");
			Thread.sleep(15000);
			driver.findElement(objmap.getLocator("CATPreviewCourse")).click();
			Thread.sleep(15000);
			...
			TakeScreenshot("Fail-Method-runCATpreviewCourseLaunch");
			logFAIL("The method runCATpreviewCourseLaunch failed (error)");
			assertFail();
		}
	}
		...
		try {
			...
			for (String key : templateTypeHashMap.keySet()) {
				if(checkVisibilityOfTemplate(0, key)){
					...
					TestNG testng = new TestNG();
					List<String> suites = Lists.newArrayList();
					suites.add(System.getProperty("user.dir")+"\\resource\\TestNGxml\\Sanity\\"+key+".xml");
	...
	public static void writeExcelData(int ColumnNumber, int RowNumber, String Label) throws Exception {
		try {
			FileInputStream fs = new FileInputStream(System.getProperty("user.dir")+"\\testData\\"+ExcelName+".xls");
			Workbook existingWorkbook = Workbook.getWorkbook(fs);
			WritableWorkbook workbookCopy = Workbook.createWorkbook(new File(System.getProperty("user.dir")+"\\testData\\"+ExcelName+".xls"), existingWorkbook);
